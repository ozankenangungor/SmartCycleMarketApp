...
try {
    ...
    if(!authToken) return sendErrorRes(res,"unauthoriezed ",403);
    
    const token = authToken.split("Bearer ")[1]
    const payload = jwt.verify(token,process.env.JWT_SECRET!) as {id: string}
    
    const user = await UserModel.findById(payload.id)
    if(!user) return sendErrorRes(res,"unauthosriez request",403);
    ...
    if(error instanceof TokenExpiredError) return sendErrorRes(res,"Süre bitti",401);
    if(error instanceof JsonWebTokenError) return sendErrorRes(res,"Unauthorized access",401);
    next(error)
}
}
export const isValidPassResetToken:RequestHandler = async (req,res,next)=>{
    ...
    **/
    ...
    if(!resetPassToken) return sendErrorRes(res,"un req",403);
    
    const matched =  await resetPassToken.compareToken(token)
    if(!matched) return sendErrorRes(res,"un req2",403);
