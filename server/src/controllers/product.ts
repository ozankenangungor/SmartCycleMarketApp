...
/**
1. User must be authenticated / isAuth
2. User can upload images as well / fileparser
3. Validate incoming data /validate(newProductSchema)
4. Create product 
...
if(isMultipleImages && images.length > 5) return sendErrorRes(res,"Image files can not be more than 5",422);
...
export const updateProduct : RequestHandler = async (req,res) => {
    ...
    */
    ...
    const { name, price, category, despcription, purchasingDate, thumbnail} = req.body
...
export const deleteProduct: RequestHandler = async(req,res) => {
    ...
    */
    ...
    if(images.length) {
        const ids = images.map(({id}) => { return id} );
...
export const deleteProductImage : RequestHandler = async (req,res) => {
    ...
    */
    ...
    if(!isValidObjectId(productId)) return sendErrorRes(res,' Invalid product', 422);
