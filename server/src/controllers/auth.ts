...
if(!password) return sendErrorRes(res,"Password is missing!",422)
 ...
 // 4. Check if we already have account with same user.
  ...
  if(existingUser) return sendErrorRes(res,"Email is already use",401);

   const user =  await UserModel.create({name,email,password})
 // 6. Generate and Store verification token.
   const token =  crypto.randomBytes(36).toString('hex')

   await AuthVerificationModel.create({owner:user._id,token:token})
...
if(!authToken) return sendErrorRes(res,"unauthoriezed request!",403)

  const isMatched =  await authToken.compareToken(token)
  if(!isMatched) return sendErrorRes(res,'unauhtorized request',403)

 await UserModel.findByIdAndUpdate(id,{verified:true})

 await AuthVerificationModel.findByIdAndDelete(authToken._id)

 res.json({message:'E postanız onaylandı'})
...
if(!isMatched) return sendErrorRes(res,"Email/password mismatch2",403);
...
export const generateVerificationLink : RequestHandler = async (req,res)=>{
 ...
 await AuthVerificationModel.create({owner:id,token})
...
if(!refreshToken) return sendErrorRes(res,'unauthorized reqrefresh',403);
const payload = jwt.verify(refreshToken,process.env.JWT_SECRET!) as {id:string}
if(!payload.id) return sendErrorRes(res,"unauthorized request payload",401)
...
if(!user) {
  ...
  return sendErrorRes(res,"unauthorized compromised",401)
}
...
export const signOut:RequestHandler = async(req,res)=>{
  ...
  */
  ...
  if(!user) return sendErrorRes(res,"unauthorized user not found",403);
...
export const generateForgetPassLink : RequestHandler = async(req,res)=>{
  ...
  */
  ...
  if(!user) return sendErrorRes(res,"Account not found password reset",404);
  ...
  // Send response back
  res.json({message:'Please check your email for reset password '})
}
...
export const updatePassword : RequestHandler = async (req,res)=>{
  ...
  5. If there is no user or user isuing the same password send error res
  ...
  */
  ...
  if(match) return sendErrorRes(res,"The new password must be different",422);
...
/**
1. User must be logged in (authanticated)
...
if(typeof name !== 'string' || name.trim().length<3) return sendErrorRes(res,'Invalid name!',422);
 ...
 */
 ...
 if(Array.isArray(avatar)) return sendErrorRes(res,'Multiple files are not allowed',422);

 if(!avatar.mimetype?.startsWith("image")) return sendErrorRes(res,'Invalid image file',422);

 const user = await UserModel.findById(req.user.id);

 if(!user) return sendErrorRes(res,'User not found (file)',422);

 // remove avatar file
 if(user.avatar?.id) await cloudUploader.destroy(user.avatar.id) ;


 // upload avatar file
const {secure_url: url, public_id: id} = await cloudUploader.upload(avatar.filepath,{width:300,height:300,crop:'thumb',gravity:'face'})
...
export const sendPublicProfile : RequestHandler = async (req,res)=>{
  ...
  if(!isValidObjectId(profileId)) return sendErrorRes(res,'Invalid profile id',422);
